...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const res = await makeRequest.get("/notifications");
        // Add read flag if not provided by backend
        const notifs = res.data.map((n) => ({ ...n, read: n.read ?? false }));
  ...
  const handleNotificationClick = (index, notifId) => {
    ...
    makeRequest.put(`/notifications/${notifId}/read`).catch((err) =>
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n, i) => (
                <div
                  ...
                  key={i}
                  onClick={() => handleNotificationClick(i, n.id)}
                >
                  <img
                    src={n.fromPic ? "/upload/" + n.fromPic : "/default-profile.png"}
                    alt=""
...
//     if (notif.postId) {
//       navigate(`/post/${notif.postId}`); // Go to post page
//     } else {
//       navigate(`/profile/${notif.fromUserId}`); // fallback to profile
//     }
...
//         <Link to={`/profile/${currentUser.id}`} key={currentUser.id} style={{ textDecoration: "none" }}>
//           <div className="user">
//             <img
//               src={currentUser?.profilePic ? "/upload/" + currentUser.profilePic : "/default-profile.png"}
//               alt={""}
//             />
//             <span>{currentUser.name}</span>
//           </div>
